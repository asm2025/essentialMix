...
/// <summary>
/// Win32PipeStream is an odd little hybrid between the client and server ends of a NamedPipe,
...
/// Server side
/// -----------
/// 1. Call Win32PipeStream.Create, specify inbound, outbound, or both
...
/// </summary>
public class Win32PipeStream : Stream
{
	...
	public Win32PipeStream(string name, FileAccess mode)
	{
		name = name?.Trim();
	...
	public Win32PipeStream(IntPtr handle, FileAccess mode)
	{
	...
	public override long Length => throw new NotSupportedException("Win32PipeStream does not support seeking");

	public override long Position
	{
		get => throw new NotSupportedException("Win32PipeStream does not support seeking");
		set { }
	}

	public override void Flush()
	{
		if (_readHandle.IsInvalidHandle()) throw new ObjectDisposedException("Win32PipeStream", "The stream has already been closed");
	...
	public override int Read(byte[] buffer, int offset, int count)
	{
		...
		if (_readHandle.IsZero()) throw new ObjectDisposedException("Win32PipeStream", "The stream has already been closed");
		...
		if (!Win32.ReadFile(_readHandle, buf, (uint)count, out uint read, IntPtr.Zero)) throw new Win32Exception(Marshal.GetLastWin32Error());
		if (offset == 0) return (int)read;
		Buffer.BlockCopy(buf, 0, buffer, offset, (int)read);
		return (int)read;
	}
	...
	public override void SetLength(long length) { throw new NotSupportedException("Win32PipeStream doesn't support SetLength"); }

	public override void Write(byte[] buffer, int offset, int count)
	{
		buffer.Length.ValidateRange(offset, ref count);
		if (!CanWrite) throw new NotSupportedException("The stream does not support writing");
		if (_writeHandle.IsZero()) throw new ObjectDisposedException("Win32PipeStream", "The stream has already been closed");
	...
	public override long Seek(long offset, SeekOrigin origin) { throw new NotSupportedException("Win32PipeStream doesn't support seeking"); }
	...
	[NotNull]
	public static Win32PipeStream Create(string name, Direction mode, bool inherit = true, uint bufferSize = 0)
	{
		name = name?.Trim();
		...
		if (name == null)
		{
			...
			self = new Win32PipeStream {Name = null, _readHandle = rh, _writeHandle = wh};
		}
		else
		{
			...
			self = new Win32PipeStream {Name = name, _readHandle = handle, _writeHandle = handle};
		}
