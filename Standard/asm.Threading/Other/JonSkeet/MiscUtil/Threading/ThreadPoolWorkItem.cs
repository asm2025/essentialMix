...
{
	/// <summary>
	/// Class encapsulating an item of work to be executed in a ThreadPool.
	/// </summary>
	public class ThreadPoolWorkItem
	{
		...
		public ThreadPoolWorkItem(object id, bool preserveParameters, bool cloneParameters,
			int priority, [NotNull] Delegate target, params object[] parameters)
		{
			ID = id;
			Priority = priority;
			PreserveParameters = preserveParameters;
			Target = target ?? throw new ArgumentNullException(nameof(target));
			if (parameters != null) Parameters = cloneParameters ? (object[])parameters.Clone() : parameters;
		}
		...
		/// the work item's execution. The ID of the constructed work item == null,
		...
		/// takes no parameters.
		/// </param>
		public ThreadPoolWorkItem([NotNull] Delegate target, params object[] parameters)
		...
		/// ThreadPool. However, this means that the contents cannot
		...
		{
			object[] p = Parameters;
			if (!PreserveParameters) Parameters = null;

			// Should be faster than dynamic invoke
			if (Target is ThreadStart start)
			{
				start();
			}
			else
