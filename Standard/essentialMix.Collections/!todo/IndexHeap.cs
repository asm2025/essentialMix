...
namespace essentialMix.Collections
{
	...
	[Serializable]
	public abstract class IndexHeap<TNode, TKey, TValue> : IKeyedHeap<TNode, TKey, TValue>, IReadOnlyCollection<TValue>, ICollection
		where TNode : KeyedBinaryNode<TNode, TKey, TValue>
	{
		...
		{
			[NonSerialized]
			private readonly IndexHeap<TNode, TKey, TValue> _indexHeap;
			private readonly int _version;

			private int _index;
			private bool _done;

			internal Enumerator([NotNull] IndexHeap<TNode, TKey, TValue> indexHeap)
			{
				_indexHeap = indexHeap;
				_version = indexHeap._version;
				_index = -1;
				Current = default(TValue);
				_done = indexHeap.Count == 0;
			}
			...
			object IEnumerator.Current
			{
				get
				{
					if (!_index.InRange(0, _indexHeap.Count)) throw new InvalidOperationException();
					return Current;
				}
			}
			...
			{
				if (_version != _indexHeap._version) throw new VersionChangedException();
				if (_done) return false;

				_index = _indexHeap.NextValidIndex(_index);

				if (_index == _indexHeap.Count)
				{
					Current = default(TValue);
					_done = true;
					return false;
				...
				Current = _indexHeap._nodes[_index].Value;
				_index++;
				return true;
			}
		...
		/// <inheritdoc />
		protected IndexHeap(int capacity)
			: this(capacity, null)
		{
		}

		/// <inheritdoc />
		protected IndexHeap(IComparer<TKey> comparer)
			: this(0, comparer)
		{
		}

		protected IndexHeap(int capacity, IComparer<TKey> comparer)
		{
		...
		/// <inheritdoc />
		protected IndexHeap([NotNull] IEnumerable<TValue> enumerable)
			: this(enumerable, null)
		{
		}

		/// <inheritdoc />
		protected IndexHeap([NotNull] IEnumerable<TValue> enumerable, IComparer<TKey> comparer)
			: this(0, comparer)
		{
			Add(enumerable);
		}
		...
		public virtual bool Equals(IndexHeap<TNode, TKey, TValue> other)
		{
	...
	[DebuggerTypeProxy(typeof(IndexHeap<,>.DebugView))]
	[Serializable]
	public abstract class IndexHeap<TKey, TValue> : IndexHeap<KeyedBinaryNode<TKey, TValue>, TKey, TValue>
	{
		internal sealed class DebugView : Dbg_IndexMinDebugView<KeyedBinaryNode<TKey, TValue>, TKey, TValue>
		{
			public DebugView([NotNull] IndexHeap<TKey, TValue> heap)
				: base(heap)
			{
			}
		}
		...
		/// <inheritdoc />
		protected IndexHeap([NotNull] Func<TValue, TKey> getKeyForItem)
			: this(getKeyForItem, 0, null)
		{
		}

		/// <inheritdoc />
		protected IndexHeap([NotNull] Func<TValue, TKey> getKeyForItem, int capacity)
			: this(getKeyForItem, capacity, null)
		{
		}

		protected IndexHeap([NotNull] Func<TValue, TKey> getKeyForItem, IComparer<TKey> comparer)
			: this(getKeyForItem, 0, comparer)
		{
		}

		protected IndexHeap([NotNull] Func<TValue, TKey> getKeyForItem, int capacity, IComparer<TKey> comparer)
			: base(capacity, comparer)
		{
			_getKeyForItem = getKeyForItem;
		}

		protected IndexHeap([NotNull] Func<TValue, TKey> getKeyForItem, [NotNull] IEnumerable<TValue> enumerable)
			: this(getKeyForItem, enumerable, null)
		{
		}

		protected IndexHeap([NotNull] Func<TValue, TKey> getKeyForItem, [NotNull] IEnumerable<TValue> enumerable, IComparer<TKey> comparer)
			: this(getKeyForItem, 0, comparer)
		{
			Add(enumerable);
		}
	...
	[DebuggerTypeProxy(typeof(IndexHeap<>.DebugView))]
	[Serializable]
	public abstract class IndexHeap<T> : IndexHeap<KeyedBinaryNode<T>, T, T>
	{
		internal sealed class DebugView : Dbg_IndexMinDebugView<KeyedBinaryNode<T>, T, T>
		{
			public DebugView([NotNull] IndexHeap<T> heap)
				: base(heap)
			{
			}
		}
		...
		protected IndexHeap(IComparer<T> comparer)
			: this(0, comparer)
		{
		}

		protected IndexHeap(int capacity, IComparer<T> comparer)
			: base(capacity, comparer)
		{
		}

		protected IndexHeap([NotNull] IEnumerable<T> enumerable)
			: this(enumerable, null)
		{
		}

		protected IndexHeap([NotNull] IEnumerable<T> enumerable, IComparer<T> comparer)
			: this(0, comparer)
		{
			Add(enumerable);
		}
