...
{
	public sealed class ThresholdTaskGroupQueue : ProducerConsumerThresholdQueue, IDisposable
	{
		...
		public ThresholdTaskGroupQueue(CancellationToken token = default(CancellationToken))
			: this(null, token)
		{
		}

		public ThresholdTaskGroupQueue(ProducerConsumerQueueOptions options, CancellationToken token = default(CancellationToken))
			: base(options, token)
		{
		...
		private void Consume()
		{
			...
			try
			{
				...
				{
					n = 0;
					if (HasThreshold) tasks[n++] = Task.Delay(ThresholdInternal).ConfigureAwait();
					...
					if (IsDisposedOrDisposing || Token.IsCancellationRequested || !tasks.WaitSilently(Token)) return;
				}

				while (!IsDisposedOrDisposing && !Token.IsCancellationRequested && _queue.Count > 0)
				{
					if (HasThreshold && n == 0) tasks[n++] = Task.Delay(ThresholdInternal).ConfigureAwait();
