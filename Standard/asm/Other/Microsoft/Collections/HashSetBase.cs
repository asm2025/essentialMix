	...
	public abstract class HashSetBase<T> : ISet<T>, IReadOnlyCollection<T>, ISerializable, IDeserializationCallback
	{
		...
		public struct Enumerator : IEnumerator<T>, IEnumerator
		{
			...
			object IEnumerator.Current
			{
				get
				{
					if (_index == 0 || _index == _set._lastIndex + 1) throw new InvalidOperationException();
					return Current;
				}
			}
		...
		public virtual void GetObjectData(SerializationInfo info, StreamingContext context)
		{
			...
			info.AddValue(COMPARER_NAME, Comparer, typeof(IEqualityComparer<T>));
		...
		public virtual void OnDeserialization(object sender)
		{
			...
			Comparer = (IEqualityComparer<T>)_siInfo.GetValue(COMPARER_NAME, typeof(IEqualityComparer<T>));
			_freeList = -1;
		...
		public void TrimExcess()
		{
			...
			else
			{
				...
				int newSize = asmMath.GetPrime(Count);
		...
		{
			int newSize = asmMath.ExpandPrime(Count);
		...
		{
			Contract.Assert(asmMath.IsPrime(newSize), "New size is not prime!");
