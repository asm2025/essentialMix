...
namespace Other.Microsoft.Windows
{
	...
	public class Desktop : Disposable, ICloneable
	{
		...
		public bool Close()
		{
			...
			{
				// close the desktop.
				bool result = Win32.CloseDesktop(_desktop);
		...
		public bool Show()
		{
			...
			bool result = Win32.SwitchDesktop(_desktop);
		...
		[NotNull]
		public static Desktop GetCurrent() { return new Desktop(Win32.GetThreadDesktop(Thread.CurrentThread.ManagedThreadId)); }
		...
		public static bool SetCurrent([NotNull] Desktop desktop) { return desktop.IsOpen && Win32.SetThreadDesktop(desktop.DesktopHandle); }
		...
		public static string GetDesktopName(IntPtr desktopHandle)
		{
			...
			int needed = 0;
			Win32.GetUserObjectInformation(desktopHandle, Win32.UOI_NAME, IntPtr.Zero, 0, ref needed);

			// get the name.
			IntPtr ptr = Marshal.AllocHGlobal(needed);
			bool result = Win32.GetUserObjectInformation(desktopHandle, Win32.UOI_NAME, ptr, needed, ref needed);
		...
		public static Process[] GetInputProcesses()
		{
			...
			foreach (Process process in processes)
			{
				...
				foreach (ProcessThread pt in process.Threads)
				{
					...
					if (GetDesktopName(Win32.GetThreadDesktop(pt.Id)) == currentDesktop)
					{
