...
using asm.Exceptions.Caching;
using asm.Extensions;
using asm.Threading;
using JetBrains.Annotations;

namespace asm.Other.JonSkeet.MiscUtil
{
	/// <inheritdoc />
	/// <summary>
	...
	public sealed class BufferManager : IBufferManager
	{
		/// <inheritdoc />
		/// <summary>
		/// Provides options for BufferManager.
		/// </summary>
		public sealed class Options : ICloneable
		{
			...
			public int MaxBuffersPerSizeBand
			{
				...
				set
				{
					if (value < 1) throw new ArgumentOutOfRangeException(nameof(value), "Must have at least 1 buffer per size band");
			...
			/// On construction of the BufferManager, this must not be greater than
			...
			public int MinBufferSize
			{
				...
				set
				{
					if (value < 1) throw new ArgumentOutOfRangeException(nameof(value), "Minimum buffer size must be at least 1");
			...
			public double ScalingFactor
			{
				...
				set
				{
					if (value < 1.25) throw new ArgumentOutOfRangeException(nameof(value), "Scaling factor must be at least 1.25.");
			...
			/// On construction of the BufferManager, this must not be less than
			/// MinBufferSize.
			/// </summary>
			public int MaxBufferSize
			{
				...
				set
				{
					if (value <= 0) throw new ArgumentOutOfRangeException(nameof(value), "Maximum buffer size must be non-negative");
			...
			/// </summary>
			public BufferUnavailableAction ActionOnBufferUnavailable { get; set; } = BufferUnavailableAction.ReturnUncached;

			/// <summary>
			/// Strongly typed Clone implementation.
			/// </summary>
			/// <returns>A clone of these options</returns>
			public Options Clone() { return this.CloneMemberwise(); }
		...
		/// <summary>
		/// Options configuration this manager.
		/// </summary>
		...
		/// as a timeout.
		/// </summary>
		private readonly SyncLock _padlock = new SyncLock("Lock for BufferManager", 5000);
		...
		public BufferManager([NotNull] Options options)
		{
			// Make sure the settings are available to all threads.
			using (_padlock.Lock())
			{
		...
		/// </summary>
		/// <exception cref="T:asm.Exceptions.BufferAcquisitionException">
		/// If the buffer could
		...
		/// </exception>
		/// <exception cref="T:global::System.ArgumentOutOfRangeException">If minimumSize is less than 0.</exception>
		[NotNull]
		public IBuffer GetBuffer(int minimumSize)
		{
			if (minimumSize < 0) throw new ArgumentOutOfRangeException(nameof(minimumSize), "minimumSize must be greater than or equal to 0");
			if (minimumSize > _options.MaxBufferSize) throw new BufferAcquisitionException($"Requested buffer {minimumSize} is larger than maximum buffer size {_options.MaxBufferSize}");
			...
			while (true)
			{
				Buffer ret = FindAvailableBuffer(listIndex, size);
				if (ret != null) return ret;

				switch (_options.ActionOnBufferUnavailable)
				{
					case Options.BufferUnavailableAction.ReturnUncached:
						return new Buffer(minimumSize, _options.ClearAfterUse);
				...
				if (size == _options.MaxBufferSize) return new Buffer(minimumSize, _options.ClearAfterUse);
				size = CalculateNextSizeBand(size);
				listIndex++;
			}
		}
		...
		private int CalculateNextSizeBand(int size) { return (int)Math.Ceiling(Math.Min(_options.MaxBufferSize, size * _options.ScalingFactor)); }
		...
		private Buffer FindAvailableBuffer(int listIndex, int size)
		{
			using (_padlock.Lock())
			{
				// Make sure there will be an entry, even if it's null
				...
				Buffer[] buffers = _bufferBands[listIndex];

				if (buffers == null)
				{
					buffers = new Buffer[_options.MaxBuffersPerSizeBand];
				...
				for (int i = 0; i < buffers.Length; i++)
				{
					...
					if (buffers[i] == null)
					{
						buffers[i] = new Buffer(size, _options.ClearAfterUse);
						return buffers[i];
					}
