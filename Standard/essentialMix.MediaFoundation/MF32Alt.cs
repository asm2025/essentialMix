...
// still have to use the workarounds.
...
namespace essentialMix.MediaFoundation.Alt
{
	...
	public interface IMFSourceReaderAsync
    {
		...
		[MethodImpl(MethodImplOptions.PreserveSig)]
		int SetCurrentPosition([MarshalAs(UnmanagedType.LPStruct), In] Guid guidTimeFormat, [MarshalAs(UnmanagedType.LPStruct), In] ConstPropVariant varPosition);
		...
		int GetPresentationAttribute(
			...
			[MarshalAs(UnmanagedType.CustomMarshaler, MarshalCookie = "IMFSourceReaderAsync.GetPresentationAttribute", MarshalTypeRef = typeof (PVMarshaler)), In, Out] Variant pvarAttribute);
    }
