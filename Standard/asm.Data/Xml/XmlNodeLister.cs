...
{
	public sealed class XmlNodeLister : Disposable, IReadOnlyList<XmlNode>, IEnumerator<XmlNode>, IEnumerator
	{
		private readonly XPathNodeIterator _iterator;
		private readonly IList<XmlNode> _list;

		private XPathNodeIterator _impl;
		private bool _done;

		public XmlNodeLister([NotNull] XPathNodeIterator iterator)
		{
			_iterator = iterator;
			_impl = _iterator.Clone();
			_list = new List<XmlNode>(_impl.Count);
		...
		/// <inheritdoc />
		public XmlNode Current => Item(Index);
		...
		private int MoveTo(int index)
		{
			...
			while (!_done && _list.Count <= index)
			{
				...
				_list.Add(_impl.Current.GetNode());
