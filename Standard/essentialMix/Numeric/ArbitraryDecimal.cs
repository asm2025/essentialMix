...
namespace essentialMix.Numeric
{
	...
	internal class ArbitraryDecimal
	{
		...
		public override string ToString()
		{
			...
			if (_decimalPoint == 0) return new string(digitString);

			// Fairly simple case - nothing after the decimal
			// point, but some 0s to add, eg value=350
			if (_decimalPoint < 0) return new string(digitString) + new string('0', -_decimalPoint);

			// Nothing before the decimal point, eg 0.035
			if (_decimalPoint >= digitString.Length) return "0." + new string('0', _decimalPoint - digitString.Length) + new string(digitString);
